...
PYTHON_SRC_ROOT = REPO_ROOT / "packages" / "python" / "src"
...
def load_extract_module(module_name: str):
    ...
    try:
        ...
        if exc.name in {"synapt_extract", f"synapt_extract.{module_name}"}:
            pytest.fail(
                "Expected Python package `packages/python/src/synapt_extract/` with "
                f"module `{module_name}.py`, but it does not exist yet."
            )
        raise
